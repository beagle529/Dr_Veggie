...
# 定義問題數據
quiz_data = [
    {"q": "哪種111蔬菜是橘色的？", "choices": ["胡蘿蔔", "菠菜", "馬鈴薯"], "answer": "胡蘿蔔", "image": "https://upload.wikimedia.org/wikipedia/commons/6/63/Carrot.jpg"},
            ...
            <div class="row g-4">
                ...
                <div class="col-md-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p class="card-title">{{ loop.index }}. {{ question["q"] }}</p>
                            {% for choice in question["choices"] %}
                            <div class="form-check">
                                <input class="form-check-input" type="radio" id="q{{ loop.index }}-{{ loop.index1 }}" name="q{{ loop.index }}" value="{{ choice }}" required>
                                <label class="form-check-label" for="q{{ loop.index }}-{{ loop.index1 }}">{{ choice }}</label>
                            </div>
                            {% endfor %}
                        </div>
                    </div>
...
def challenge():
    return render_template_string(challenge_template, questions=quiz_data)
